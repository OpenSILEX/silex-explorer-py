    ...
    try:
        ...
        for obj in scientific_objects:
            ...
            if obj.get("hasFactorLevel"):
                for factor_level in obj["hasFactorLevel"]:
                    ...
                    row[factor_name] = (
                        row.get(factor_name, "") + (", " if row.get(factor_name) else "") + factor_level_label
                    )
            ...
            if obj.get("hasGermplasm"):
                for idx, germplasm in enumerate(obj["hasGermplasm"], start=1):
                    ...
                    if germplasm.get("fromAccession"):
                        row[f"Accession_{idx}"] = ", ".join(
                            accession["label"] for accession in germplasm["fromAccession"]
                        )
        ...
        if germplasm_type is not None:
            ...
            if germplasm_name is not None:
                ...
                name_columns = [
                    col for col in df.columns if col.startswith("Germplasm_") or col.startswith(germplasm_type)
                ]
                if name_columns:
                    # Create a condition to check if germplasm_name exists in any of these columns
                    name_filter = df[name_columns].apply(lambda row: germplasm_name in row.values, axis=1)
