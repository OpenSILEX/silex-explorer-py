...
def get_environmental_data_by_facility(
    session, facility_name, var_env=None, date_beginning=None, date_end=None, csv_filepath=None
):
    """
    ...
    try:
        ...
        # Add date conditions to _operators
        if (
            date_beginning and date_end and date_beginning == date_end
        ):  # Both dates are equal and not None:  # Case where start and end date are the same
        ...
        while True:
            ...
            response = requests.post(
                session["url_graphql"],
                json={"query": data_query, "variables": {"filter": filter_input, "page": page, "perPage": per_page}},
